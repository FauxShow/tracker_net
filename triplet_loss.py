...
class TripletLoss(nn.Module):
    ...
    def TripletSemiHardLoss(self, embeddings, labels):
        ...
        pdist_matrix = pairwise_distance_torch(embeddings, self.device)
        ...
        triplet_loss = (torch.max(torch.mul(loss_mat, mask_positives), torch.tensor([0.]).to(self.device))).sum() / num_positives
        triplet_loss = triplet_loss.to(dtype=embeddings.dtype)
        return triplet_loss

    def forward(self, input, labels):
        return self.TripletSemiHardLoss(input, labels)
